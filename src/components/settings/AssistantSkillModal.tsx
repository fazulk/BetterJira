...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () => (
      <Teleport to="body">
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onMousedown={handleBackdropMouseDown}>
          <div class="flex w-full max-w-2xl flex-col rounded-xl border border-white/[0.1] bg-[#16171b] p-5 shadow-2xl shadow-black/50">
            <div class="mb-4 flex items-start justify-between gap-3">
              ...
              {props.skill && (
                <div class="relative shrink-0">
                  ...
                  {menuOpen.value && (
                    <div class="absolute right-0 top-8 z-10 w-36 rounded-md border border-white/[0.1] bg-[#1d1e23] py-1 shadow-xl shadow-black/40">
                      <button type="button" class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs text-rose-300 transition hover:bg-white/[0.05]" onClick={() => emit('delete')}>
            ...
            <div class="mt-4 flex items-center justify-end gap-2">
              <button type="button" class="rounded-full border border-white/[0.1] px-4 py-1.5 text-sm text-slate-300 transition hover:bg-white/[0.05]" onClick={() => emit('close')}>
                Cancel
              </button>
              <button type="button" class="rounded-full bg-accent-indigo px-4 py-1.5 text-sm font-medium text-white transition hover:bg-accent-indigo/90 disabled:cursor-not-allowed disabled:opacity-50" disabled={!canSave.value} onClick={save}>
                Save
              </button>
            </div>
          </div>
