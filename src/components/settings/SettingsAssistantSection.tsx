  ...
  setup() {
    ...
    return () => (
      <section class="mx-auto max-w-3xl space-y-5">
        <div>
          <h2 class="text-xl font-semibold text-slate-100">Assistant</h2>
        ...
        <div class="rounded-lg border border-white/[0.06] bg-white/[0.02]">
          <div class="grid gap-0 divide-y divide-white/[0.06] md:grid-cols-2 md:divide-x md:divide-y-0">
            <label class="block p-4">
              ...
              <select value={settings.value.provider} name="assistant-provider" class="w-full rounded-md border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-sm text-slate-200 outline-none transition focus:border-white/[0.16] focus:bg-white/[0.06]" onChange={handleProviderChange}>
            ...
            <label class="block p-4">
              ...
              <select value={settings.value.model} name="assistant-model" class="w-full rounded-md border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-sm text-slate-200 outline-none transition focus:border-white/[0.16] focus:bg-white/[0.06]" onChange={handleModelChange}>
        ...
        <div class="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
            ...
            {ASSISTANT_REASONING_LEVELS.map(level => (
              <button
                ...
                class={[
                  ...
                  settings.value.reasoning === level ? 'bg-white/[0.08] text-slate-100' : 'text-slate-500 hover:text-slate-300',
                ]}
          ...
          <p class="mt-2 text-[11px] text-slate-600">Higher effort spends more time reasoning before responding.</p>
        </div>

        <div class="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
          <div class="mb-3 flex items-start justify-between gap-3">
            <div>
              <h3 class="text-sm font-medium text-slate-200">System prompt</h3>
            ...
            <button type="button" class="shrink-0 rounded-md border border-white/[0.08] px-2.5 py-1.5 text-xs font-medium text-slate-300 transition hover:bg-white/[0.05] disabled:cursor-not-allowed disabled:opacity-40" disabled={isSavingPrompt.value || (isPromptDefault.value && !isPromptDirty.value)} onClick={resetPrompt}>
              Reset to default
            </button>
          </div>
          <textarea
          ...
          <div class="mt-3 flex items-center justify-between gap-3">
            {promptFeedback.value
              ? <p class={['text-xs', promptFeedback.value.kind === 'error' ? 'text-rose-300' : 'text-emerald-300']}>{promptFeedback.value.message}</p>
              : <span />}
            <button type="button" class="shrink-0 rounded-md bg-accent-indigo px-3 py-1.5 text-xs font-medium text-white transition hover:bg-accent-indigo/90 disabled:cursor-not-allowed disabled:bg-white/[0.06] disabled:text-slate-600" disabled={isSavingPrompt.value || !isPromptDirty.value} onClick={savePrompt}>
        ...
        <div>
          ...
          <p class="mt-0.5 text-xs text-slate-500">Reusable prompts you can attach to a chat message from the composer.</p>
          <div class="mt-3 overflow-hidden rounded-lg border border-white/[0.06] bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3 border-b border-white/[0.06] px-4 py-3">
              ...
              <button type="button" class="flex h-7 w-7 items-center justify-center rounded-md text-slate-400 transition hover:bg-white/[0.05] hover:text-slate-200" aria-label="Add skill" title="Add skill" onClick={() => openSkillModal(null)}>
                <Icon name="lucide:plus" class="h-4 w-4" aria-hidden="true" />
              </button>
            </div>
            {skills.value.length === 0 && (
              <p class="px-4 py-4 text-xs text-slate-600">No skills yet. Add one to reuse a prompt in the assistant chat.</p>
            )}
            {skills.value.map(skill => (
              <button key={skill.id} type="button" class="block w-full border-b border-white/[0.04] px-4 py-3 text-left transition last:border-b-0 hover:bg-white/[0.03]" onClick={() => openSkillModal(skill)}>
                <span class="block text-sm text-slate-200">{skill.name}</span>
                {skill.updatedAt && <span class="mt-0.5 block text-xs text-slate-600">{formatSkillUpdatedAt(skill.updatedAt)}</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div class="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
          <div class="mb-3 flex items-center justify-between gap-3">
            <div>
              <h3 class="text-sm font-medium text-slate-200">CLI availability</h3>
              <p class="mt-0.5 text-xs text-slate-500">The assistant uses the same login as your interactive CLI session.</p>
            </div>
            {isLoadingProviders.value && <span class="text-xs text-slate-500">Checking...</span>}
          </div>
          <div class="grid gap-2 md:grid-cols-2">
            {providers.map(provider => (
              ...
              >
                ...
                <p class="mt-1 text-[11px] leading-relaxed opacity-80">{getAvailabilityDetail(provider)}</p>
              </div>
            ))}
          </div>
          <div class={[
            'mt-2 rounded-md border px-3 py-2 text-xs',
            isAcliAvailable.value
              ? 'border-white/[0.08] bg-white/[0.035] text-slate-300'
              : 'border-amber-400/20 bg-amber-500/10 text-amber-100',
          ]}
          >
            <div class="flex items-center justify-between gap-3">
              <span class={['font-medium', isAcliAvailable.value ? 'text-slate-200' : 'text-amber-100']}>Atlassian CLI (acli)</span>
              <span class="text-[10px] uppercase tracking-[0.16em]">{isAcliAvailable.value ? 'Available' : 'Unavailable'}</span>
            </div>
            <p class="mt-1 text-[11px] leading-relaxed opacity-80">{acliAvailabilityDetail.value}</p>
            {!isAcliAvailable.value && acliInstallInstructions.value.length > 0 && (
              <div class="mt-2 space-y-1 text-[11px] leading-relaxed">
                  ...
                  {acliInstallInstructions.value.map(instruction => (
                    <li key={instruction}>
                      {instruction.startsWith('https://')
                        ? (
                            <a href={instruction} target="_blank" rel="noreferrer" class="underline decoration-amber-200/40 underline-offset-2 hover:text-amber-50">
                              Atlassian CLI install guide
                            </a>
                          )
                        : <code class="rounded bg-black/20 px-1 py-0.5">{instruction}</code>}
                    </li>
                  ))}
                </ol>
              </div>
          ...
          {feedback.value && <p class="mt-3 rounded-md border border-rose-400/20 bg-rose-500/10 px-3 py-2 text-xs text-rose-200">{feedback.value}</p>}
        </div>
