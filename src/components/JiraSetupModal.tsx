...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => isVisible.value && (
      <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 px-4 py-8 backdrop-blur-sm">
        <div class="animate-slide-up flex max-h-[calc(100vh-4rem)] w-full max-w-xl flex-col overflow-hidden rounded-lg border border-white/[0.08] bg-surface-0 shadow-xl shadow-black/40">
          {setupStep.value === 'form' && (
            <>
              ...
              <form class="min-h-0 flex-1 overflow-y-auto px-4 py-3" onSubmit={handleSubmit}>
                ...
                {errorMessage.value && (
                  <p class="mt-4 rounded-md border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-[13px] leading-5 text-rose-200">
                    {errorMessage.value}
                  </p>
                )}

                <div class="mt-4 flex items-center justify-between gap-3 border-t border-white/[0.06] pt-4">
                  <p class="text-[11px] leading-4 text-slate-600">Credentials are stored locally in .data/settings.json.</p>
                  <button type="submit" disabled={!canSubmit.value} class="inline-flex h-8 shrink-0 items-center rounded-md bg-accent-indigo px-3 text-[13px] font-medium text-white transition hover:bg-accent-indigo/90 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          {setupStep.value === 'success' && (
            <div class="px-4 py-6" aria-live="polite">
              ...
              <div class="mt-6 flex items-center justify-center">
                <button type="button" class="inline-flex h-8 items-center rounded-md bg-accent-indigo px-3 text-[13px] font-medium text-white transition hover:bg-accent-indigo/90" onClick={continueToSettings}>
