      ...
      <div class="w-[420px] max-w-[92vw] overflow-hidden rounded-xl border border-white/[0.08] bg-[#16181d] text-[#d7d8dc] shadow-2xl shadow-black/50">
        ...
        <div class="flex items-center gap-2 border-b border-white/[0.06] px-3 py-3">
          {SPACE_COLOR_SWATCHES.map(swatch => (
            ...
            >
              {props.color === swatch.value && <Icon name="lucide:check" class="h-3.5 w-3.5 text-white" aria-hidden="true" />}
            </button>
          ))}
          ...
          <label
            ...
            title="Custom color"
          >
            {!isPresetColor.value && <Icon name="lucide:check" class="h-3.5 w-3.5 text-white drop-shadow" aria-hidden="true" />}
            <input type="color" class="absolute inset-0 cursor-pointer opacity-0" value={props.color} aria-label="Custom color" onInput={onCustomColorInput} />
          </label>
        </div>
        ...
        <div class="max-h-[280px] overflow-y-auto px-2 pb-3">
          {filteredIcons.value.length === 0
            ? (
                <div class="px-2 py-6 text-center text-[12px] text-[#6f727b]">
                  No icons match "
                  {search.value}
                  "
                </div>
                ...
                <div class="grid grid-cols-9 gap-1">
                  {filteredIcons.value.map(name => (
                    <button
                      key={name}
                      type="button"
                      class={['flex aspect-square items-center justify-center rounded-md transition hover:bg-white/[0.08]', props.icon === name && 'bg-white/[0.1] ring-1 ring-inset ring-white/20']}
                      title={formatIconLabel(name)}
                      aria-label={formatIconLabel(name)}
                      onClick={() => emit('update:icon', name)}
                    >
                      <Icon name={`lucide:${name}`} class="h-4 w-4" style={{ color: props.color }} aria-hidden="true" />
                    </button>
                  ))}
                </div>
              )}
